...
/*
  * G630 tp interference fm,so ofilm change its module design.
  * We use different product ID to distinguish old and new design.
  * To make sure that all tp update the right firmware.
*/
/*Use this to get module name*/
#define FW_OFILM_HW "G630HW-000" //g630 ofilm old product ID
#define FW_OFILM_MV "G630MV-000" //g630 ofilm new product ID
#define FW_JUNDA_HW "G630HW-005" //g630 junda product ID
...
static char * get_cof_module_name(u8 module_id)
{
	...
	switch(module_id)
	{
		case TP_COF_ID_OFILM:
			/*Add for g630*/
			if (0 == strncasecmp(FW_OFILM_MV, fwu->product_id,SYNAPTICS_RMI4_PRODUCT_ID_SIZE))
			{
				return "ofilm2";
			}
			else
	...
	else if(0 == strncasecmp(FW_OFILM_HW, fwu->product_id,SYNAPTICS_RMI4_PRODUCT_ID_SIZE))
	{
		return "ofilm";
	}
	else if(0 == strncasecmp(FW_JUNDA_HW, fwu->product_id,SYNAPTICS_RMI4_PRODUCT_ID_SIZE))
	{
		return "junda";
	}
	else
