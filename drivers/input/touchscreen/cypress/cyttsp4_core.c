				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && cd) {
		...
		if (*blank == FB_BLANK_UNBLANK)
			cyttsp4_core_rt_resume(cd->dev);
		else if (*blank == FB_BLANK_POWERDOWN)
			cyttsp4_core_rt_suspend(cd->dev);
	}
	return 0;
}

static void configure_sleep(struct cyttsp4_core_data *cd)
{
	int retval = 0;
	...
	retval = fb_register_client(&cd->fb_notif);
	if (retval)
		tp_log_err(
...
static int cyttsp4_core_release(struct cyttsp4_core *core)
{
	...
	if (cd->pdata->init)
       ...
       /*
#if !defined(CONFIG_HAS_EARLYSUSPEND) && defined(CONFIG_FB)
	fb_unregister_client(&cd->fb_notif);
#endif
       */
