...
static int ga = 515;     //A  light  coefficient
static int gc = 300;    //C  light  coefficient
static int gd = 200;    //D  light  coefficient
static int A_light_division = 217;
static int D_light_division = 50;
        ...
        case ECS_IOCTL_APP_SET_LFLAG:   /* app set  light sensor flag */
        {
			...
			if (set_flag)
			{
				value_reg0 = reg0_value | (set_flag << APDS9930_AEN_BIT_SHIFT);
           		ret = set_9930_register(this_aps_data, APDS9930_ENABLE_REG, value_reg0, 0);
				set_9930_register(this_aps_data,APDS9930_ATIME_REG,aTime,0);
				schedule_delayed_work(&this_aps_data ->als_dwork, msecs_to_jiffies(200));
			}
			else
			{
				value_reg0 = reg0_value & APDS9930_REG0_AEN_OFF;
           			ret = set_9930_register(this_aps_data, APDS9930_ENABLE_REG, value_reg0, 0);
				cancel_delayed_work(&this_aps_data->als_dwork);
			}
            if (ret)
            {
...
static int luxcalculation(int cdata, int irdata)
{
    ...
    */
    ...
    if(cdata < A_light_division*irdata/100)            //A light
    {

         first_half = iac*ga*DF/100;
	}
    else if(cdata <= D_light_division*irdata/10)    // D light
    {
	  first_half = iac*gd*DF/100;
    }
    else                                                                    //C light 
    {
        first_half = iac*gc*DF/100;
    }
...
static void aps_9930_work_func(struct work_struct *work)
{
    ...
    else if (atomic_read(&p_flag) && !(status & APDS9930_STATUS_PROXIMITY_BIT) )
    {
        ...
        else
        {
            ...
            APS9930_DBG("%s, line %d: [APS_ERR]p_flag is open, but no prox int(STATUS=0x%x,ENABLE=0x%x,PDATA=%d,PILT=%d,PIHT=%d)\n", \
				...
				struct aps9930_hw_platform_data *pdata)
{
	...
	else
	{
      ...
      rc = of_property_read_u32(np, "aps,a_light", &temp_val);
      if (rc)
      {
		dev_err(dev, "Unable to read a_light\n");
		return rc;
      }
      else
      {
              pdata->a_light = (int)temp_val;
      }

      rc = of_property_read_u32(np, "aps,c_light", &temp_val);
      if (rc)
      {
		dev_err(dev, "Unable to read c_light\n");
		return rc;
      }
      else
      {
              pdata->c_light = (int)temp_val;
	       printk("[aps_parse_dt],c_light=%d\n",pdata->c_light);
      }

     rc = of_property_read_u32(np, "aps,d_light", &temp_val);
     if (rc)
      {
		dev_err(dev, "Unable to read d_light\n");
		return rc;
      }
      else
      {
              pdata->d_light = (int)temp_val;
      }
...
static void aps9930_als_work_handler(struct work_struct  *als_dwork)
{
     struct aps_data *aps = container_of((struct delayed_work *)als_dwork, struct aps_data, als_dwork);
     int cdata = 0;
     int irdata = 0;
     int lux_is_valid =1;
     int lux =0;
   ...
   schedule_delayed_work(&aps ->als_dwork, msecs_to_jiffies(200));
}
    ...
    struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ga = platform_data -> a_light;
    gc = platform_data -> c_light;
    gd = platform_data -> d_light;
...
/*8930 mate aps power*/
    ...
    INIT_DELAYED_WORK(&aps->als_dwork,aps9930_als_work_handler);
    aps->client = client;
