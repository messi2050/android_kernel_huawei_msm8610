...
#define IMX134_SUNNY_P8N09C_GPIO 98  //id pin
#define IMX134_SUNNY_P8N09C_MODULE_ID  1  //module id
...
static struct msm_sensor_power_setting imx134_sunny_p8n09c_power_down_setting[] = {
	{
		.seq_type = SENSOR_I2C_MUX,
		.seq_val = 0,
		.config_val = 0,
...
***************************************************************************/
static int imx134_sunny_p8n09c_match_module(struct msm_sensor_ctrl_t *s_ctrl)
{
    int rc = 0; //return value
    int pullup_read = -ENODEV; //pullup id value
    int pulldown_read = -ENODEV ; //pulldown id value
    int module_gpio = IMX134_SUNNY_P8N09C_GPIO ; //gpio num of id pin
    pr_info("%s %d :  sensor_name=%s,  \n",  __func__, __LINE__,IMX134_SUNNY_P8N09C_SENSOR_NAME);
    ...
    else//connect 
    {
        ...
        if(IMX134_SUNNY_P8N09C_MODULE_ID == pulldown_read)
        {
            //rc = IMX134_SUNNY_P8N09C_MODULE_ID detect success
            rc = FUNC_SUCCESS;
...
***************************************************************************/
static int imx134_sunny_p8n09c_add_project_name(struct msm_sensor_ctrl_t *s_ctrl)
{
    int rc = 0;
    ...
    pr_info("%s %d : imx134_sunny_p8n09c_add_project_name sensor_project_name=%s \n",  __func__, __LINE__,
            s_ctrl->sensordata->sensor_info->sensor_project_name);
    /*add the app_info*/
    rc = app_info_set("camera_main", "imx134_sunny");
    if(FUNC_SUCCESS != rc )
    {//set fail
        pr_err("%s %d : imx134_sunny_p8n09c_add_project_name fail! \n",  __func__, __LINE__);
    }
    else
    {//set ok
        pr_info("imx134_sunny_p8n09c_add_project_name OK\n");
    }
...
static struct msm_sensor_fn_t imx134_sunny_p8n09c_sensor_func_tbl = {
	...
	.sensor_match_module = imx134_sunny_p8n09c_match_module,
#ifdef CONFIG_HUAWEI_KERNEL_CAMERA
	.sensor_add_project_name = imx134_sunny_p8n09c_add_project_name,
#endif
};
...
static struct msm_sensor_ctrl_t imx134_sunny_p8n09c_s_ctrl = {
	...
	.power_setting_array.power_down_setting = imx134_sunny_p8n09c_power_down_setting,
	.power_setting_array.size_down = ARRAY_SIZE(imx134_sunny_p8n09c_power_down_setting),
