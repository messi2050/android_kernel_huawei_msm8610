...
/*mclk enable before pull reset when power on, mclk disable after avdd/iovdd/dvdd disable when power off*/
	...
	for (index = 0; index < power_setting_array->size; index++) {
			...
			if(s_ctrl->power_down_setting_array.power_setting){
				...
				for(i=0; i<power_down_setting_array->size; i++){
					...
					if(power_setting->seq_val == power_down_setting->seq_val){
						for(j=0; j<s_ctrl->sensordata->power_info.clk_info_size; j++)
						{
		...
		case SENSOR_I2C_MUX:
			if (data->power_info.i2c_conf && data->power_info.i2c_conf->use_i2c_mux)
				imx219_liteon_sensor_enable_i2c_mux(data->power_info.i2c_conf);
			break;
		default:
...
power_up_failed:
	...
	for (index = 0; index < power_setting_array->size; index++){
...
#endif
			{
				//power up failed,need set gpio low
				gpio_set_value_cansleep(
					data->power_info.gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val], 
					GPIO_OUT_LOW);
			}
		...
		case SENSOR_I2C_MUX:
			if (data->power_info.i2c_conf && data->power_info.i2c_conf->use_i2c_mux)
				imx219_liteon_sensor_disable_i2c_mux(data->power_info.i2c_conf);
			break;
		default:
			pr_err("%s error power seq type %d\n", __func__,
				power_setting->seq_type);
...
#endif
			{
				//Should be config_val,but not seq_val
				gpio_set_value_cansleep(
					data->power_info.gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val],
					power_setting->config_val);
			}
